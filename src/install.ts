...
import { fetchOpts, parseSlug, dirSlug } from './api.js'
...
export function skillDir(skillsDir: string, slug: string): string {
  ...
  const target = resolve(root, dirSlug(slug))
...
export async function installSkill(slug: string, cfg: PluginConfig, deps: InstallDeps = defaultDeps, signal?: AbortSignal): Promise<InstallResult> {
  const id = parseSlug(slug)
  const files = await downloadSkillFiles(id, cfg, deps, signal)
  ...
  const base = dirSlug(id)
  const staging = await mkdtemp(join(cfg.skillsDir, `.tmp-${base}-`))
  try {
...
async function downloadSkillFiles(slug: string, cfg: PluginConfig, deps: InstallDeps = defaultDeps, signal?: AbortSignal): Promise<Record<string, Buffer>> {
  const id = parseSlug(slug)
  const zipUrl = `${cfg.apiBase.replace(/\/$/, '')}/api/v1/skills/${encodeURIComponent(id)}/download`
  ...
  if (!/zip|octet-stream/i.test(contentType) && body.subarray(0, 2).toString() !== 'PK') {
    throw new Error(`WittyHub 下载不是 zip: ${id}`)
  }
...
export async function uninstallSkill(slug: string, skillsDir: string): Promise<{ slug: string; path: string }> {
  ...
  if (!hasSkill) throw new Error(`未安装或不含 SKILL.md: ${dirSlug(id)}`)
  await rm(target, { recursive: true, force: true })
  return { slug: dirSlug(id), path: target }
}
