...
const SORTS: SortBy[] = ['updated_at', 'download_count']
...
export function sanitizePatch(raw: Record<string, unknown>): Partial<PluginConfig> {
  ...
  if (Number.isFinite(max) && max >= 1) out.maxResults = Math.min(Math.floor(max), 96)
...
export function withDefaults(config: Partial<PluginConfig>): PluginConfig {
  return {
    apiBase: config.apiBase || 'https://skillhub.openeuler.org',
